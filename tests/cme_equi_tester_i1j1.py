...
# Note:
# Suppose it_worker_types = 1, it_occ_types = 1
# There is a single wage to solve for.
# log(L1/L0) = itc + slp * log(wage)
# - Suppose L1 + L0 = 4.24353289 and L1 = 0.5 because Y/Z = 0.5

Ltotal = 4.24353289
Lab = 0.5
itc = -1.443274175544382
slp = 0.477319860608414
wage = np.exp(((np.log(Lab / (Ltotal - Lab)) - itc) / slp))
print(f'{wage=}')

# suppose I know the correct wage
Lwkr0 = Ltotal / (np.exp(itc + slp * np.log(wage)) + 1)
Lwkr1 = Ltotal - Lwkr0
print(f'{Lwkr1=} and {Lwkr0=}')
