# Use cme_equi_solve_nest_test to generate random input parameters and solve model
# Store the output data as panda dataframes, converted from dicts
# Store dataframe to folder as csv
# Load in CSV file, convert to dataframe, convert to DICT
...
dc_ces_flat, dc_supply_lgt, \
    ...
    cme_equi_solve_nest.cme_equi_solve_nest_test(
        it_fixed_group=11, verbose=True)
...
pd_qtlv_all_demand, __ = cme_inpt_parse.cme_parse_qtwg_mat(
    dc_ces_flat, ar_splv_totl_acrs_i)
...
dc_supply_lgt, pd_qtlv_all_supply = cme_splv_opti.cme_supply_lgt_solver(
    dc_supply_lgt)
...
if bl_pandas_out:
    ...
    # Path to load
    spt_path_demand = spt_data
    spt_path_supply = spt_data
    dc_load_path = {'spt_path_demand': spt_path_demand,
                    'snm_file_demand': snm_file_demand,
                    'spt_path_supply': spt_path_supply,
                    'snm_file_supply': snm_file_supply}

    # 6. Solve the model with paraemters loaded in from CSV file
    start = timeit.default_timer()
    dc_ces_flat_resolve, dc_supply_lgt_resolve, \
        dc_equi_solv_sfur_resolve, _, _, _ = \
        cme_equi_solve_nest.cme_equi_solve_nest_test(
            dc_load_path=dc_load_path, verbose=True)
    stop = timeit.default_timer()
    print('Time: ', stop - start)
    pd_qtlv_all_resolve = dc_equi_solv_sfur_resolve['pd_qtlv_all']
    pd_wglv_all_resolve = dc_equi_solv_sfur_resolve['pd_wglv_all']

    # check if solutions match
    pd_qtlv_equi_vs_resolve = pd_qtlv_all - pd_qtlv_all_resolve
    fl_diff_equi_qt_resolve = np.sum(np.sum(np.abs(pd_qtlv_equi_vs_resolve)))
    if verbose:
        print(f'{fl_diff_equi_qt_resolve=}')
    pd_wglv_equi_vs_resolve = pd_wglv_all - pd_wglv_all_resolve
    fl_diff_equi_wg_resolve = np.sum(np.sum(np.abs(pd_wglv_equi_vs_resolve)))
    if verbose:
        print(f'{fl_diff_equi_wg_resolve=}')
