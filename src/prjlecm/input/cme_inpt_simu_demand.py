...
def cme_simu_demand_ces_inner_dict(
    ...
    dc_type_return = {
        ...
        "wkr": int,
        "occ": int,
        st_node_value_index: int,
        st_node_value: ast.literal_eval,
        st_node_value_bounds: list,
        st_node_var: str,
        st_node_lab: str,
        st_share_param: float,
        "pwr": float,
    ...
    dc_val_return = {
        ...
        "wkr": it_wkr,
        "occ": it_occ,
        st_node_value_index: it_nvi,
        st_node_value: gn_nvl,
        st_node_value_bounds: ls_nvb,
        st_node_var: st_nvr,
        st_node_lab: st_nlb,
        st_share_param: fl_shr,
        "pwr": fl_pwr,
        "ipt": ar_it_ipt,
        "qty": fl_qty,
        "wge": fl_wge,
...
def cme_simu_demand_mnest_wkr_occ_keys(
    ar_it_chd_tre=[2, 2, 2, 2], ar_it_occ_lyr=[2, 3], verbose=False
):
    ...
    ls_chd_uqe_idx_all = [np.arange(it_chd_tre) for it_chd_tre in ar_it_chd_tre]
    mt_occ_wkr_mesh = np.array(np.meshgrid(*ls_chd_uqe_idx_all)).T.reshape(
        -1, len(ar_it_chd_tre)
    )
    ...
    ar_it_col_sel_wkr = [
        idx for idx in np.arange(len(ar_it_chd_tre)) if idx not in ar_it_col_sel_occ
    ]
...
def cme_simu_demand_params_ces_single(
    it_worker_types=2,
    it_occ_types=2,
    fl_power_min=0.1,
    fl_power_max=0.8,
    ...
    if bl_simu_params:
        ...
        fl_power = (
            np.random.uniform(low=fl_power_min, high=fl_power_max, size=(1,))
        ).item()
    else:
        mt_rand_coef_shares = np.reshape(
            [None] * (it_worker_types * it_occ_types), [it_worker_types, it_occ_types]
        )
        fl_power = None
    ...
    for it_worker_type_ctr in np.arange(it_worker_types):
        for it_occ_type_ctr in np.arange(it_occ_types):
            ...
            _, dc_cur_input, _ = cme_simu_demand_ces_inner_dict(
                it_lyr=1,
                it_prt=0,
                it_wkr=it_worker_type_ctr,
                it_occ=it_occ_type_ctr,
    ...
    _, dc_ces[0], _ = cme_simu_demand_ces_inner_dict(
        it_lyr=0, fl_pwr=fl_power, ar_it_ipt=ar_it_inputs
    )
...
def cme_simu_demand_params_ces_nested(
    ar_it_chd_tre=[2, 2, 3],
    ar_it_occ_lyr=[1],
    fl_power_min=0.1,
    fl_power_max=0.8,
    ...
    ls_mnest_wrk_occ_keys = cme_simu_demand_mnest_wkr_occ_keys(
        ar_it_chd_tre=ar_it_chd_tre, ar_it_occ_lyr=ar_it_occ_lyr, verbose=verbose_debug
    )
    ...
    dc_ces_keys_tree = dict(
        [
            (
                it_tre_ctr + it_lower_count,
    ...
    for it_parent, ar_it_children in dc_ces_keys_tree.items():
        ...
        if bl_simu_params:
            ...
            ar_rand_coef_shares_children = ar_rand_coef_shares_children / np.sum(
                ar_rand_coef_shares_children
            )
        else:
            # All None Values
            ar_rand_coef_shares_children = ar_rand_coef_shares[ar_it_children]
    ...
    for it_brh_chd_layer_rev in reversed(ar_it_chd_tre[0:-1]):
        ...
        for it_branch_ctr in np.arange(it_tre_up_cnt):
            ...
            it_within_cnt = (
                ...
                - it_tre_up_cnt_last
                - 1
                + it_branch_ctr * it_brh_chd_layer_rev
            )
            ...
            for it_cur_chd in np.arange(it_brh_chd_layer_rev):
                ...
                ar_it_ipt = [
                    key
                    for key, nest in dc_ces_flat.items()
    ...
    fl_shr = None
    ar_it_ipt = [
        key for key, nest in dc_ces_flat.items() if nest["prt"] == it_top_key_index
    ]
